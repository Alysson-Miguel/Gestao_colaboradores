...
export default function CargosPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const list = await CargosAPI.listar({ limit: 1000, search: query || undefined });
          ...
          <div className="flex items-center justify-between">
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                ...
                <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: textMain }}>Cargos</h1>
              </div>
          ...
          <section style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, overflow: "hidden" }}>
            {loading ? (
              ...
              <div style={{ padding: "60px 20px", textAlign: "center", color: textMuted, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                <Briefcase size={40} strokeWidth={1.2} style={{ opacity: 0.3 }} />
                <p style={{ fontSize: 14 }}>Nenhum cargo encontrado</p>
              </div>
            ) : (
              <CargoTable
                cargos={filtered}
