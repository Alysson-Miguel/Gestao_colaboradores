...
export default function EstacoesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const list = await EstacoesAPI.listar({ limit: 1000, search: query || undefined });
      setEstacoes(list);
    } catch (err) {
      ...
      <MainLayout>
        ...
        <main className="px-8 py-6 space-y-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                ...
                <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: textMain }}>Estações</h1>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{
                ...
                <Radio size={12} />
                {estacoes.length} estação{estacoes.length !== 1 ? "ões" : ""}
              </span>
              <button
          ...
          <section style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, overflow: "hidden" }}>
            {loading ? (
              <div style={{ padding: "40px 20px", textAlign: "center", color: textMuted }}>Carregando estações…</div>
            ) : estacoes.length === 0 ? (
              <div style={{ padding: "60px 20px", textAlign: "center", color: textMuted, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                <Radio size={40} strokeWidth={1.2} style={{ opacity: 0.3 }} />
