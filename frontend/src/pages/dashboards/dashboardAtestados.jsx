...
export default function DashboardAtestados() {
  ...
  try {
    ...
    setTopOfensores(
      resRisco.data?.data?.topOfensores ??
      resRisco.data?.topOfensores ??
      []
    )
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={Array.isArray(tendencia) ? tendencia : []}>
                ...
                <XAxis dataKey="data" tick={{ fill: "#BFBFC3", fontSize: 12 }} />
    ...
    <div className="h-60">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <XAxis
            dataKey="name"
            tick={{ fill: "#BFBFC3", fontSize: 11 }}
          />

          <YAxis
            allowDecimals={false}
            tick={{ fill: "#BFBFC3", fontSize: 12 }}
            domain={[0, maxValue]}   // 🔥 REMOVE o +1
            interval={0}             // 🔥 força mostrar só valores reais
          />
...
function PieBlock({ data }) {
  ...
  const renderLabel = ({percent, value }) => {
    if (percent < 0.05) return null // não mostrar se muito pequeno
    return `${value}`
  }
  ...
  return (
    <div className="h-62 flex flex-col items-center justify-center">
      ...
      {/* Legenda customizada */}
<div className="text-xs mt-3 space-y-2 w-full px-4">
  {safeData.map((item, i) => {
...
function TopOfensoresTable({ rows, loading }) {
  ...
  if (!rows?.length) {
    return <p className="text-sm text-white/60">Sem ofensores no período.</p>
  }
...
function BarBlockHorizontal({ data }) {
    ...
    // reduz nome para PRIMEIRO + ÚLTIMO
...
function BarBlockHorizontalCID({ data }) {
  ...
  // 🔥 Altura dinâmica baseada na quantidade de CIDs
  const chartHeight = safeData.length * 36
