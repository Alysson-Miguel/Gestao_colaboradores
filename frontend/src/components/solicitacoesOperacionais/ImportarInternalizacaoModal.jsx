...
function baixarModeloCsv() {
  const cabecalho = ["CPF", "Nova Empresa", "Nova Matrícula", "Motivo", "Confirmar Mesmo Assim"];
  const linhas = [
    ["000.000.000-00", "SPX", "AC0000000000", "Exemplo de motivo da internalização", "NAO"],
  ];
  ...
  link.download = "modelo_internalizacao.csv";
  link.href = url;
  link.click();
  URL.revokeObjectURL(url);
}

export function ImportarInternalizacaoModal({ onClose }) {
  ...
  const enviar = async () => {
    ...
    setResultado(null);
    try {
      const data = await SolicitacoesOperacionaisAPI.importarInternalizacaoLote(arquivo);
      setResultado(data);
      setArquivo(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-default shrink-0">
          <div>
            <h2 className="font-semibold text-base">Importar Internalização em Massa</h2>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted">
              Colunas: <span className="text-page">CPF, Nova Empresa, Nova Matrícula, Motivo, Confirmar Mesmo Assim</span>.
              <br />
              "Nova Empresa" deve bater com a razão social cadastrada (ex: SPX).
              <br />
              "Confirmar Mesmo Assim" = <span className="text-page">SIM</span> força a criação mesmo se o colaborador não atender aos critérios de elegibilidade — do contrário a linha vira erro.
            </p>
          </div>
