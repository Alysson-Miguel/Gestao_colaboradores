...
function baixarModeloCsv() {
  const cabecalho = ["CPF", "Data", "Dia Completo", "Quantidade de Horas", "Hora de Entrada", "Motivo"];
  const linhas = [
    ["000.000.000-00", "15/09/2026", "NAO", "4", "13:00", "Exemplo: 4h de banco de horas a partir das 13h"],
    ["000.000.000-00", "16/09/2026", "SIM", "", "", "Exemplo: dia completo de banco de horas"],
  ];
  ...
  link.download = "modelo_banco_horas.csv";
  link.href = url;
  link.click();
  URL.revokeObjectURL(url);
}

export function ImportarBancoHorasModal({ onClose }) {
  ...
  const enviar = async () => {
    ...
    setResultado(null);
    try {
      const data = await SolicitacoesOperacionaisAPI.importarBancoHorasLote(arquivo);
      setResultado(data);
      setArquivo(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-default shrink-0">
          <div>
            <h2 className="font-semibold text-base">Importar Banco de Horas em Massa</h2>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted">
              Colunas: <span className="text-page">CPF, Data (dd/mm/aaaa), Dia Completo, Quantidade de Horas, Hora de Entrada, Motivo</span>.
              <br />
              Dia Completo aceito: SIM ou NÃO. Se SIM, deixe Quantidade de Horas e Hora de Entrada em branco.
              <br />
              Se NÃO, Quantidade de Horas (ex: 4) e Hora de Entrada (HH:MM, ex: 13:00) são obrigatórios.
            </p>
          </div>
