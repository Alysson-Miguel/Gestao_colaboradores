/**
 * Controller de Aprovadores de Solicitações Operacionais
 * Cadastro simples usado para autorizar quem pode aprovar/reprovar
 * Solicitações Operacionais (vínculo por e-mail do usuário logado).
 *
...
exports.listAprovadores = async (req, res) => {
  try {
    ...
    const aprovadores = await prisma.aprovadorOperacional.findMany({
      where,
    ...
    console.error("❌ listAprovadores (operacional):", err);
...
exports.createAprovador = async (req, res) => {
  try {
    ...
    const existente = await prisma.aprovadorOperacional.findUnique({
    ...
    const aprovador = await prisma.aprovadorOperacional.create({
      data: {
        nome: nome.trim(),
    ...
    console.error("❌ createAprovador (operacional):", err);
...
exports.updateAprovador = async (req, res) => {
  try {
    ...
    const aprovador = await prisma.aprovadorOperacional.findUnique({
    ...
    const atualizado = await prisma.aprovadorOperacional.update({
      where: { idAprovador: Number(id) },
      data: {
    ...
    console.error("❌ updateAprovador (operacional):", err);
...
exports.deleteAprovador = async (req, res) => {
  try {
    ...
    await prisma.aprovadorOperacional.update({
      where: { idAprovador: Number(id) },
      data: { ativo: false },
    });
    ...
    console.error("❌ deleteAprovador (operacional):", err);
